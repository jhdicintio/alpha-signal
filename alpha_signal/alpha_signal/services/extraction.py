...
def extract_article(
    ...
    callers processing batches can skip failures gracefully. When
    *system_prompt* is provided, it overrides the extractor default.
    """
    try:
        result = extractor.extract(article, system_prompt=system_prompt)
        logger.info(
    ...
    *,
    ...
    for i, article in enumerate(eligible, 1):
        ...
        extraction = extract_article(
            article, extractor, system_prompt=system_prompt
        )
            ...
            async with sem:
                ...
                try:
                    extraction = await extractor.extract_async(
                        article, system_prompt=system_prompt
                    )
                except Exception:
                    logger.exception("extraction failed for article %s", article.source_id)
                    with lock:
