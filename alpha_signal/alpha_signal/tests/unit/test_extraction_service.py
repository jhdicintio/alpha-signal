...
class TestExtractArticle:
    def test_returns_extraction(self):
        ...
        extractor.extract.assert_called_once_with(
            ARTICLE_WITH_ABSTRACT, system_prompt=None
        )
...
class TestExtractBatch:
    ...
    def test_skips_failed_extractions(self):
        ...
        def side_effect(article, system_prompt=None):
            nonlocal call_count
            call_count += 1
            if call_count == 1:
    ...
    def test_concurrent_skips_failed_extractions(self):
        ...
        async def async_side_effect(article, system_prompt=None):
            nonlocal call_count
            call_count += 1
            if call_count == 1:
                raise RuntimeError("boom")
...
class TestExtractBatchBudget:
    ...
    def test_stops_mid_batch_when_budget_exhausted(self):
        ...
        def extract_and_record(article, system_prompt=None):
            nonlocal call_count
            call_count += 1
